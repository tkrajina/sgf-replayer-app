...
import { GAMES_STORAGE } from "./storage";
	...
	constructor() {
		...
		if (localStorage.getItem(GAMES)) {
			...
			for (const game of games) {
				GAMES_STORAGE.save(game);
			}
		}
		
		if (!GAMES_STORAGE.listAll()?.length) {
