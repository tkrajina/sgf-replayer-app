	...
	constructor() {
		...
		if (localStorage.getItem(GAMES)) {
			...
			for (const game of games) {
				STORAGE_SERVICE.GAMES_STORAGE.save(game);
			}
		}
		
		if (!STORAGE_SERVICE.GAMES_STORAGE.listAll()?.length) {
...
`)
	...
	loadGames() {
		return STORAGE_SERVICE.GAMES_STORAGE.listAll();
	}
	...
	loadGame(id: string) {
		return STORAGE_SERVICE.GAMES_STORAGE.byId(id);
	}
	...
	delete(game: Game) {
		STORAGE_SERVICE.GAMES_STORAGE.delete(game);
	}
