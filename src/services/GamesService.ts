...
import { APPLICATION_SERVICE } from "./ApplicationService";
...
class GamesService {
	...
	constructor() {
		...
		if (localStorage.getItem(GAMES)) {
			const games = (JSON.parse(localStorage.getItem(GAMES)) || []) as Game[];
			localStorage.removeItem(GAMES);
			for (const game of games) {
				APPLICATION_SERVICE.GAMES_STORAGE.save(game);
			}
		}
		
		if (!APPLICATION_SERVICE.GAMES_STORAGE.listAll()?.length) {
...
`)
	...
	loadGames() {
		return APPLICATION_SERVICE.GAMES_STORAGE.listAll();
	}
	...
	loadGame(id: string) {
		return APPLICATION_SERVICE.GAMES_STORAGE.byId(id);
	}
	...
	delete(game: Game) {
		APPLICATION_SERVICE.GAMES_STORAGE.delete(game);
	}
